...
var DragControls = function (_objects, _camera, _domElement) {
	...
	function onMouseMove(event) {
		...
		if (_selected && scope.enabled) {
			...
			scope.dispatchEvent({ type: 'drag', intersection: _intersection.sub(_offset).applyMatrix4(_inverseMatrix) });
	...
	function onMouseDown(event) {
		...
		if (_intersections.length > 0) {
			...
			let current = Fbx.getByUuid(_selected.uuid)
			while (!current && _selected.parent) {
			...
			scope.dispatchEvent({ type: 'dragstart', object: _selected, item: current });
